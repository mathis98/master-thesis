...
def calculate_mAP(image_embeddings, caption_embeddings, ground_truth_labels, top_k=10):
	...
	for i, ground_truth in enumerate(ground_truth_labels):
		...
		true_positives = torch.sum(binary_labels * (top_k_indices[i] == torch.arange(len(image_embeddings), device=caption_embeddings.device).unsqueeze(0).unsqueeze(2).expand_as(top_k_indices[i])[:, :top_k]))
