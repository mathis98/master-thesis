...
class FullPipeline(pl.LightningModule):
			...
			for batch in dataloader:
				...
				for idx, embed in zip(true_label_value.tolist(), image_embed):
					if idx in unique_embeddings:
						...
						if not true_label:
							idx = idx // 100
						temp_labels = torch.cat([temp_labels, torch.tensor(idx, dtype=torch.long).to(image_embed.device).unsqueeze(0)], dim=0)
	...
	def test_step(self, batch, batch_idx):
		...
		mAP = self.shared_step(batch, validation=False)
	...
	def on_test_epoch_end(self):
		...
		self.log('avg_test_mAP', avg_mAP, batch_size=self.batch_size, prog_bar=True, sync_dist=True)
